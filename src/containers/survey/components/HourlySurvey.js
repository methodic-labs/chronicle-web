...
const HourlySurvey = ({
  ...
  const getInstructionText = () => {
    switch (surveyStep) {
      case SELECT_CHILD_APPS:
        return t(TranslationKeys.INSTRUCTIONS_SELECT_CHILD_APPS);
      case SELECT_SHARED_APPS:
        return t(TranslationKeys.INSTRUCTIONS_SELECT_SHARED_APPS);
      case RESOLVE_SHARED_APPS:
        return t(TranslationKeys.INSTRUCTIONS_RESOLVE_SHARED_APPS);
      case RESOLVE_OTHER_APPS:
        return t(TranslationKeys.INSTRUCTIONS_RESOLVE_OTHER_APPS);
      default:
        return '';
    }
  };
  ...
  const nextButtonText = useMemo(() => {
    if (isFinalStep) return t(TranslationKeys.SUBMIT);
    if (surveyStep === INTRO) return t(TranslationKeys.BEGIN_SURVEY);
    return t(TranslationKeys.NEXT);
