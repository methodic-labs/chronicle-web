...
import ConfirmSurveySubmissionModalI18n from './components/ConfirmSurveySubmissionModalI18n';
import HourlySurveyI18n from './components/HourlySurveyI18n';
import HourlySurveyDispatch, { ACTIONS } from '../components/HourlySurveyDispatch';
import HourlyUsageSurveyAppBarI18n from './components/HourlyUsageSurveyAppBarI18n';
import InstructionsModalI18n from './components/InstructionsModalI18n';
import SubmissionSuccessfulI18n from './components/SubmissionSuccessfulI18n';
...
const HourlyAppUsageSurveyI18n = ({
  date,
  participantId,
  studyId,
}) => {
  ...
  if (isSuccess(submitRS) || isFailure(submitRS)) {
    return (
      <HourlySurveyDispatch.Provider value={dispatch}>
        <AppContainerWrapper>
          <HourlyUsageSurveyAppBarI18n step={step} />
          <AppContentWrapper>
            {
              isSuccess(submitRS)
  ...
  return (
    <HourlySurveyDispatch.Provider value={dispatch}>
      <AppContainerWrapper>
        <HourlyUsageSurveyAppBarI18n date={date} step={step} />
        <AppContentWrapper>
          <Card>
            <CardSegment noBleed>
              <Box mb="32px">{t(HourlySurveyTranslationKeys.SELECT_DATE_TEXT)}</Box>
              <Box maxWidth="300px">
                <DatePicker
                  ...
                  }
                  {
                    isFailure(getAppUsageSurveyDataRS) && (
                      <Box textAlign="center">
                        {t(HourlySurveyTranslationKeys.ERROR_LOADING)}
                      </Box>
                    )
                  }
                </CardSegment>
      ...
      {
        isConfirmModalVisible && <ConfirmSurveySubmissionModalI18n />
      }
      {
        isInstructionsModalVisible && <InstructionsModalI18n step={step} surveyStep={surveyStep} />
      }
    </HourlySurveyDispatch.Provider>
  );
};

export default HourlyAppUsageSurveyI18n;