// @flow

export default {
  ACTIVITY_END_TIME: 'activity_end_time',
  ADULT_MEDIA: 'adult_media',
  BED_TIME_BEFORE_ACTIVITY_DAY: 'bed_time_before_activity_day',
  BED_TIME_LAST_NIGHT: 'bed_time_last_night',
  BED_TIME_TONIGHT: 'bed_time_tonight',
  BG_AUDIO_DAY: 'bg_audio_day',
  BG_AUDIO_NIGHT: 'bg_audio_night',
  BG_MEDIA_OPTIONS: 'bg_media_options',
  BG_TV_DAY: 'bg_tv_day',
  BG_TV_NIGHT: 'bg_tv_night',
  BOOK_TITLE: 'book_title',
  BOOK_TYPE: 'book_type',
  BOOK_TYPE_OPTIONS: 'book_type_options',
  BTN_BACK: 'button_back',
  BTN_CANCEL: 'button_cancel',
  BTN_CLOSE: 'button_close',
  BTN_EDIT: 'button_edit',
  BTN_NEXT: 'button_next',
  BTN_SUBMIT: 'button_submit',
  CAREGIVER: 'caregiver',
  CAREGIVER_OPTIONS: 'caregiver_options',
  CHANGE_LANGUAGE: 'change_language',
  CHANGE_LANGUAGE_WARNING: 'change_language_warning',
  CHILD_WENT_TO_BED_LAST_NIGHT: 'child_went_to_bed_last_night',
  CHILD_WENT_TO_BED_TONIGHT: 'child_went_to_bed_tonight',
  CHILD_WOKE_UP_TODAY: 'child_woke_up_today',
  CHILD_WOKE_UP: 'child_woke_up',
  CHOOSE_APPLICABLE: 'choose_applicable',
  CHOOSE_FORMAT: 'choose_format',
  CLOCK_FORMATS: 'clock_formats',
  CONTEXTUAL_TEXT: 'contextual_text',
  DAY_END_TIME_CTX: 'day_end_time_ctx',
  DAY_END_TIME: 'day_end_time',
  DAY_OF_WEEK_CTX: 'day_of_week_ctx',
  DAY_OF_WEEK: 'day_of_week',
  DAY_START_TIME_CTX: 'day_start_time_ctx',
  DAY_START_TIME: 'day_start_time',
  DEFAULT_END_TIME: 'default_end_time',
  DEFAULT_TIME: 'default_time',
  DONT_KNOW: 'dont_know',
  ERROR_END_PAST_BEDTIME: 'error_end_past_bedtime',
  ERROR_INVALID_BED_TIME: 'error_invalid_bedtime',
  ERROR_INVALID_END_TIME: 'error_invalid_end',
  ERROR_MIN_ITEMS: 'error_min_items',
  ERROR_INVALID_URL: 'error_invalid_url',
  ERROR_RESPONSE_REQUIRED: 'error_required',
  ERROR_SUBMIT: 'error_submit',
  INTRO_TEXT_CTX: 'intro_text_ctx',
  INTRO_TEXT: 'intro_text',
  MEDIA_ACTIVITY: 'media_activity',
  MEDIA_ACTIVITY_OPTIONS: 'media_activity_options',
  MEDIA_AGE: 'media_age',
  MEDIA_AGE_OPTIONS: 'media_age_options',
  MEDIA_NAME: 'media_name',
  NEXT_ACTIVITY: 'next_activity',
  NIGHTTIME_ACTIVITY_TITLE: 'nighttime_activity_title',
  NO: 'no',
  NON_TYPICAL_DAY: 'non_typical_day',
  NON_TYPICAL_DAY_REASONS: 'non_typical_day_reasons',
  NON_TYPICAL_SLEEP_OPTIONS: 'non_typical_sleep_options',
  NON_TYPICAL_SLEEP_PATTERN: 'non_typical_sleep_pattern',
  NO_ONE: 'no_one',
  OTHER: 'other',
  OTHER_ACTIVITY: 'other_activity',
  PRIMARY_ACTIVITIES: 'primary_activities',
  PRIMARY_ACTIVITY: 'primary_activity',
  SCROLL_ACTIVITIES: 'scroll_activities',
  SECONDARY_ACTIVITY: 'secondary_activity',
  SLEEPING: 'sleeping',
  SLEEP_ARRANGEMENT: 'sleep_arrangement',
  SLEEP_ARRANGEMENT_OPTIONS: 'sleep_arrangement_options',
  SLEEP_PATTERN: 'sleep_pattern',
  SUBMISSION_ERROR_TITLE: 'submission_error_title',
  SUBMISSION_SUCCESS: 'submission_success',
  SUBMISSION_SUCCESS_TITLE: 'submission_success_title',
  SUMMARY_TITLE: 'summary_title',
  TYPICAL_DAY: 'typical_day',
  TYPICAL_DAY_CHOICES: 'typical_day_choices',
  WAKE_UP_COUNT: 'wake_up_count',
  WAKE_UP_COUNT_OPTIONS: 'wake_up_count_options',
  WAKE_UP_TIME_AFTER_ACTIVITY_DAY: 'wake_up_time_after_activity_day',
  WAKE_UP_TIME_THIS_MORNING: 'wake_up_time_this_morning',
  WAKE_UP_TIME_YESTERDAY_MORNING: 'wake_up_time_yesterday_morning',
  WEEKDAY: 'weekday',
  WEEKDAY_OPTIONS: 'weekday_options',
  YES: 'yes',
};
