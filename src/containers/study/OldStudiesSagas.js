...
//         .filter((participant) => participant.getIn([ENROLLMENT_STATUS, 0]) !== DELETE),
//       studyId,
//     }));
//   }
//   catch (error) {
...
//     );
//     let notificationsEKID = yield select(
//       (state) => state.getIn([STUDIES_REDUX_CONSTANTS.STUDIES, NOTIFICATIONS_EKID])
//     );
//
//     const entitySetIds = yield select(getSelectedOrgEntitySetIds());
//     const propertyTypeIds = yield select(selectPropertyTypeIds());
...
//     formData = updateIn(
//       formData,
//       [getPageSectionKey(1, 1), getEntityAddressKey(0, PARTICIPANTS, PERSON_ID)],
//       (val) => val.trim()
//     );
//
//     const propertyTypeIds = yield select(selectPropertyTypeIds());
