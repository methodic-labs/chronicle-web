...
describe('Select', () => {
  ...
  describe('icon', () => {
    ...
    //         render={mockComponent} />
    //   );
    //   expect(wrapper.find(SearchIcon).props().icon).toEqual(<SearchIcon />);
    // });

    test('should render custom icon', () => {
      const customIcon = () => <SearchIcon size={64} />;
      const wrapper = mount(
        <SelectController
            icon={customIcon}
            render={mockComponent} />
      );
      expect(wrapper.find(SearchIcon).prop('icon')).toEqual(<SearchIcon size={64} />);
    });
